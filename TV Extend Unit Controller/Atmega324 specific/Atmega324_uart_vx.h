...
void Usart0_WriteString(volatile char* string)
{
	...
	_delay_ms(10);// maybe find a workaround
}
...
Transmission Guidelines:
	...
	- Incoming strings must be limited with '$' to set the transmission_complete parameter (the exception is if the string equals or exceed the max string length of 20 character)
	- The receiver function must override the '$' char with the string-terminator '\0'
	- Outgoing transmissions must be terminated with '\0' or the buffer length equals or exceed the max length of 20 character. However only 20 character should be written.
